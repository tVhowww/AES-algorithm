package AES_Algorithm;

import java.util.Base64;

public class Test {

	private static int Nb, Nk, Nr; // Nb = 4; Nk: khóa; Nr: số vòng
	private static byte[][] w;

	private static int[] sbox = {  };

	private static final int[] Rcon = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };

	public static final byte[][] galois = { { 0x02, 0x03, 0x01, 0x01 }, { 0x01, 0x02, 0x03, 0x01 },
			{ 0x01, 0x01, 0x02, 0x03 }, { 0x03, 0x01, 0x01, 0x02 } };

	// dịch trái 1 byte
	private static byte[] rotWord(byte[] input) {
		byte[] tmp = new byte[input.length];
		tmp[0] = input[1];
		tmp[1] = input[2];
		tmp[2] = input[3];
		tmp[3] = input[0];
		return tmp;
	}

	private static byte getSBoxValue(byte num) {
		return (byte) sbox[num & 0xff];
	}

	// thay thế các byte trong 1 từ 4 byte bằng các
	// giá trị tương ứng từ bảng S-box
	private static byte[] subWord(byte[] input) {
		byte[] output = new byte[input.length];

		for (int i = 0; i < output.length; i++) {
			byte currentByte = input[i];

			byte substitudedByte = getSBoxValue(currentByte);
			output[i] = substitudedByte;
		}
		return output;
	}

	private static byte[][] generateSubkeys(byte[] key) {
		byte[][] keyMatrix = new byte[Nb * (Nr + 1)][4]; // 44-128, 52-192, 60-256 word để thực hiện tương ứng
		byte[] tmp = new byte[4];						 // 11-128, 13-192, 15-256 vòng mã hóa
		int index = 0;

		// tạo khóa ban đầu từ khóa key
		for (int i = 0; i < Nk; i++) {
			keyMatrix[i][0] = key[index++];
			keyMatrix[i][1] = key[index++];
			keyMatrix[i][2] = key[index++];
			keyMatrix[i][3] = key[index++];
		}
		// key expansion từ khóa ban đầu
		for (int i = Nk; i < Nb * (Nr + 1); i++) {
			for (int j = 0; j < 4; j++) { 
				tmp[j] = keyMatrix[i - 1][j];
			}
			if (i % Nk == 0) {
				tmp = rotWord(tmp);
				tmp = subWord(tmp);
				tmp[0] = (byte) (tmp[0] ^ (Rcon[i / Nk] & 0xff));
			}
			for (int j = 0; j < Nb; j++) {
				keyMatrix[i][j] = (byte) (keyMatrix[i - Nk][j] ^ tmp[j]);
			}
		}

		return keyMatrix;
	}
	
	// add round key
	private static byte[][] addRoundKey(byte[][] state, byte[][] subKeys, int round) {
		// state: mảng sau khi thực hiện mix col
		// subKeys: mảng chứa các khóa con
		// round: số vòng phải thực hiện
		byte[][] tmp = new byte[state.length][state[0].length];
		
		for (int c = 0; c < Nb; c++) {
			for (int r = 0; r < 4; r++) {
				tmp[r][c] = (byte) (state[r][c] ^ subKeys[round * Nb + c][r]);
			}
		}
		return tmp;
	}
	
	private static byte[][] subBytes(byte[][] state) {
		byte[][] result = new byte[state.length][state[0].length];
		
		for (int row = 0; row < state.length; row++) {
			for (int col = 0; col < state[0].length; col++) {
				result[row][col] = getSBoxValue(state[row][col]);
			}
		}
		return result;
	}
	
	private static byte[][] shiftRows(byte[][] state) {
		byte tmp;
	    
	    // Row 1: Shift left by 1 byte
	    tmp = state[1][0];
	    state[1][0] = state[1][1];
	    state[1][1] = state[1][2];
	    state[1][2] = state[1][3];
	    state[1][3] = tmp;

	    // Row 2: Shift left by 2 bytes
	    tmp = state[2][0];
	    state[2][0] = state[2][2];
	    state[2][2] = tmp;
	    tmp = state[2][1];
	    state[2][1] = state[2][3];
	    state[2][3] = tmp;

	    // Row 3: Shift left by 3 bytes
	    tmp = state[3][0];
	    state[3][0] = state[3][3];
	    state[3][3] = state[3][2];
	    state[3][2] = state[3][1];
	    state[3][1] = tmp;

	    return state;
		
	}
	
	private static byte[][] mixColumn(byte[][] state) {
		byte[][] result = new byte[state.length][state[0].length];
		
		for (int c = 0; c < 4; c++) {
			for (int i = 0; i < 4; i++) {
				byte tmp = 0x00;
				for (int j = 0; j < 4; j++) {
					tmp ^= multiple(galois[i][j], state[j][c]);
				}
				result[i][c] = tmp;
			}
		}
		
		return result;
	}

	private static byte multiple(byte a, byte b) {
		byte result = 0;
		for (int i = 0; i < 8; i++) {
			if ((b & 1) == 1) {
				result ^= a;
			}
			boolean highBitSet = (a & 0x80) != 0;
			a <<= 1;
			if (highBitSet) {
				a ^= 0x1b;
			}
			b >>= 1;
		}
		return result;
	}
	
	
	public static byte[] encryptBlock(byte[] input) {
		byte[] tmp = new byte[input.length];
		
		byte[][] state = new byte[4][Nb];
		for (int i = 0; i < input.length; i++) {
			state[i % 4][i / 4] = input[i];
		}
		state = addRoundKey(state, w, 0);
		
		for (int round = 1; round < Nr; round++) {
			state = subBytes(state);
			state = shiftRows(state);
			state = mixColumn(state);
			state = addRoundKey(state, w, round);
		}
		
		state = subBytes(state);
		state = shiftRows(state);
		state = addRoundKey(state, w, Nr);
		
		for (int i = 0; i < tmp.length; i++) {
			tmp[i] = state[i % 4][i / 4];
		}
		return tmp;
	}
	
	public static byte[] encrypt(byte[] in, byte[] key) {
		Nb = 4;
		Nk = key.length / 4;
		Nr = Nk + 6;
		
		int length = 0;
		byte[] padding = new byte[1];
		int i;
		length = 16 - in.length % 16;
		padding = new byte[length];
		padding[0] = (byte) 0x80;
		
		for (i = 1; i < length; i++) {
			padding[i] = 0;
		}
		byte[] tmp = new byte[in.length + length];
		byte[] block = new byte[16];
		
		int count = 0;
		w = generateSubkeys(key);
		
		for (i = 0; i < in.length + length; i++) {
			if (i > 0 && i % 16 == 0) {
				block = encryptBlock(block);
				System.arraycopy(block, 0, tmp, i - 16, block.length);
			}
			if (i < in.length) {
				block[i % 16] = in[i];
			} else {
				block[i % 16] = padding[count % 16];
				count++;
			}
		}
		if (block.length == 16) {
			block = encryptBlock(block);
			System.arraycopy(block, 0, tmp, i - 16, block.length);
		}
	
		return tmp;
	}

	public static void main(String[] args) {
		String key = "1234567812345678";
		String txt = "nguyentrunghau";
		byte[] encrypt = encrypt(txt.getBytes(), key.getBytes());
		String output = encode(encrypt);
		System.out.println(output);
		
	}

	// Hàm Base64 để mã hóa và giải mã dữ liệu văn bản sang mảng byte và ngược lại
	public static String encode(byte[] bytes) {
		return Base64.getEncoder().encodeToString(bytes);
	}
}
